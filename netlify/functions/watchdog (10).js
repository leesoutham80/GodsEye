...
module.exports.handler = schedule("*/15 * * * *", async (event) => {
  ...
  try {
    ...
    async function fetchRSS(url, name, max) {
      ...
      try {
        ...
        matches.slice(0, max || 6).forEach(item => {
          ...
          const lm = item.match(/<link[^>]*>(.*?)<\/link>/) || item.match(/<link[^>]*href=["'](.*?)["']/);
          const em = item.match(/<enclosure[^>]*type=["']audio\/[^"']*["'][^>]*url=["'](.*?)["']/);
          const dm = item.match(/<itunes:duration[^>]*>(\d+)<\/itunes:duration>/);
          
          if (tm) {
            const title = tm[1].replace(/<[^>]+>/g, "").replace(/&amp;/g,"&").substring(0, 150);
            const link = lm ? lm[1].replace(/<!\[CDATA\[|\]\]>/g, "").trim() : null;
            const audioUrl = em ? em[1] : null;
            const duration = dm ? parseInt(dm[1]) : null;
            ...
            if (title && title.length > 15) {
              items.push({
                ...
                audioUrl: audioUrl,
                duration: duration,
                isLongform: !!(audioUrl || (duration && duration > 1200)) // 20+ min = longform
              });
            }
          }
        });
    ...
    // News feeds (short-form, every 15min)
    ...
    // Podcast/long-form feeds (checked every 6hrs, queued for deep analysis)
    const longformResults = await Promise.all([
      fetchRSS("https://warontherocks.com/feed/podcast/", "wotr_podcast", 3),
      fetchRSS("https://www.cfr.org/podcasts/world-next-week/rss.xml", "cfr_podcast", 2),
      fetchRSS("https://www.brookings.edu/feed/", "brookings", 2),
      fetchRSS("https://thediplomat.com/feed/", "diplomat", 3),
    ].map(p => p.catch ? p : p.catch(() => [])));

    const results = [...newsResults, ...longformResults];

    const all = results.flat();
    
    // Separate short-form headlines from long-form content
    headlines = all.filter(item => !item.isLongform);
    const longformQueue = all.filter(item => item.isLongform);
    
    // Queue longform content for deep analysis (processed separately, max 3/day)
    if (longformQueue.length > 0) {
      try {
        const metaStore = getStore("godseye-meta");
        const queueRaw = await metaStore.get("longform_queue").catch(() => null);
        const queue = queueRaw ? JSON.parse(queueRaw) : [];
        longformQueue.forEach(item => {
        ...
        console.log("[WATCHDOG] Queued " + longformQueue.length + " longform items for analysis");
      } catch (e) {
        console.warn("[WATCHDOG] Failed to queue longform:", e.message);
      }
    }
    
    // Classify headlines
    headlines.forEach(a => { const c = classifyHeadline(a.text); a.tag = c.tag; a.intensity = c.intensity; });
    ...
    console.log("[WATCHDOG] " + activeSources + "/" + (newsResults.length + longformResults.length) + " sources active, " + all.length + " raw → " + uniqueHeadlines.length + " headlines + " + longformQueue.length + " longform");
    ...
    console.log("[WATCHDOG]", uniqueHeadlines.length, "headlines to analyze");
...
Headlines:
${uniqueHeadlines.map((h, i) => `${i + 1}. [${h.tag}${h.intensity ? "/"+h.intensity : ""}] ${h.text}`).join("\n")}`;
    ...
    // === STEP 4.5: Store headlines with full metadata for Intel View ===
    try {
      const newsStore = getStore("godseye-news");
      const newsArchiveRaw = await newsStore.get("archive").catch(() => null);
      const newsArchive = newsArchiveRaw ? JSON.parse(newsArchiveRaw) : [];
      
      // Add current headlines with full metadata + analysis
      const timestamp = new Date().toISOString();
      uniqueHeadlines.forEach((h, idx) => {
        const analysisForHeadline = suggestions.find(s => s.headline_index === idx + 1);
        newsArchive.push({
          ts: timestamp,
          text: h.text,
          url: h.url,
          ...
          signals: analysisForHeadline ? analysisForHeadline.signals.map(s => s.id) : [],
          analysis: analysisForHeadline ? JSON.stringify(analysisForHeadline.signals) : null,
          taco_phase: analysisForHeadline ? analysisForHeadline.taco_phase : null
        });
      });
      
      // Keep last 7 days of headlines (assuming 96 runs/day = ~672 headlines/day)
      const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
      const trimmedArchive = newsArchive.filter(h => new Date(h.ts).getTime() > sevenDaysAgo);
      
      await newsStore.set("archive", JSON.stringify(trimmedArchive));
      console.log("[WATCHDOG] Stored " + uniqueHeadlines.length + " headlines, archive now has " + trimmedArchive.length + " items");
    } catch (e) {
      console.warn("[WATCHDOG] Failed to store headlines:", e.message);
    }
