    ...
    for (const imp of impacts) {
      ...
      for (const sig of imp.signals) {
        if (sig.magnitude === "major" && sig.suggested_value != null) {
          // Clamp to 0-100 — defends against Haiku returning out-of-range values (e.g. 295)
          const v = Math.max(0, Math.min(100, sig.suggested_value));
          const old = state.signal_values[sig.id];
          state.signal_values[sig.id] = v;
          ...
          state.alerts.unshift({
            priority: "WARNING",
            text: `AUTO: ${sig.id} → ${v} (was ${old || "?"}) — ${sig.reason}`,
          });
          notifications.push(`${sig.id}: ${old || "?"}→${v} (${sig.reason})`);
          applied++;
        }
      }
    }
...
If no signal impact, return [].`;
  ...
  try {
    ...
    for (const imp of sourceImpacts) {
      ...
      for (const sig of imp.signals) {
        if (sig.magnitude === "major" && sig.suggested_value != null) {
          // Clamp to 0-100 — defends against Haiku returning out-of-range values
          const v = Math.max(0, Math.min(100, sig.suggested_value));
          const old = state.signal_values[sig.id];
          state.signal_values[sig.id] = v;
          state.history_log.push({
          ...
          state.alerts.unshift({
            priority: "CRITICAL",
            text: `MONITOR: ${imp.source_id || "source"} — ${sig.id} → ${v} (was ${old || "?"}) — ${sig.reason}`,
          });
          notifications.push(`[${imp.source_id}] ${sig.id}: ${old || "?"}→${v}`);
        }
      }
