 ...
 * 
 * Each signal has:
 * - keywords: terms that trigger detection
 * - up_triggers: phrases/patterns that increase signal value
 * - down_triggers: phrases/patterns that decrease signal value
 * - magnitude_rules: how to calculate impact size (default 10-20 points)
 * - auto_apply: whether to apply automatically or require review
 * - confidence_threshold: minimum confidence to trigger (0-1)
 */

export const SIGNAL_CLASSIFIER_CONFIG = {
 ...
 */
export function calculateMagnitude(signal, headline, sentiment, context) {
 ...
 */
export function determineDirection(signal, headline) {
  const config = SIGNAL_CLASSIFIER_CONFIG[signal];
