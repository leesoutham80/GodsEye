...
${headlines.map((h, i) => `${i + 1}. [${h.tag}${h.intensity ? "/"+h.intensity : ""}] ${h.text}`).join("\n")}`;
    ...
    // === LEBANON THRESHOLD MONITOR ===
    // L2 crossings, L1 sustained, L5 collapse, Syria corridor compounding
    const lebAlerts = [];
    const prevL = state._prev_lebanon || {};
    const L1 = state.signal_values.L1, L2 = state.signal_values.L2;
    const L5 = state.signal_values.L5, L7 = state.signal_values.L7;
    if (L2 != null && (prevL.L2 || 0) < 50 && L2 >= 50) {
      lebAlerts.push({ priority: "high", title: "LEBANON L2 CROSSED 50",
        body: `Hezbollah Posture ${prevL.L2}→${L2}. Symbolic fire threshold. CP Echo +20, Path E +10 (manual). Watch Brent + GMSI within 6h.`,
        alert: { type: "crit", text: `LEBANON L2 CROSSED 50 — Hezbollah posture ${L2}. Speed-bag transmission line HOT.`, priority: "CRITICAL" } });
    }
    if (L2 != null && (prevL.L2 || 0) < 75 && L2 >= 75) {
      lebAlerts.push({ priority: "urgent", title: "LEBANON L2 CROSSED 75",
        body: `Sustained kinetic exchange threshold. One step from full mobilisation. Veto Gate proximity warning.`,
        alert: { type: "crit", text: `LEBANON L2 ${L2} — sustained kinetic. Veto Gate proximity.`, priority: "CRITICAL" } });
    }
    if (L2 != null && L2 >= 100 && !state._lb_veto_notified) {
      lebAlerts.push({ priority: "urgent", title: "🔴 LEBANON VETO GATE TRIP",
        body: `L2=${L2}. Full Hezbollah mobilisation. Mode should be ESCALATION. Manual dashboard review required.`,
        alert: { type: "crit", text: `🔴 LEBANON VETO GATE TRIPPED — L2=${L2}.`, priority: "CRITICAL" } });
      state._lb_veto_notified = true;
    }
    if (L5 != null && (prevL.L5 || 100) > 30 && L5 <= 30) {
      lebAlerts.push({ priority: "high", title: "LEBANON L5 COLLAPSE",
        body: `Beirut political cohesion ${prevL.L5}→${L5}. De-escalation counterparty disappearing. Path F -5 (manual).`,
        alert: { type: "warn", text: `LEBANON L5 COLLAPSED to ${L5} — Beirut failing.`, priority: "WARNING" } });
    }
    if (L7 != null && L2 != null && L7 >= 70 && L2 >= 50 && !state._lb_compound_notified) {
      lebAlerts.push({ priority: "high", title: "LEBANON LB-5 COMPOUNDING",
        body: `L7=${L7}, L2=${L2}. Syria corridor degraded + active posture = use-it-or-lose-it stockpile pressure.`,
        alert: { type: "warn", text: `LEBANON COMPOUNDING — L7=${L7}+L2=${L2}. Stockpile pressure.`, priority: "WARNING" } });
      state._lb_compound_notified = true;
    }
    state._prev_lebanon = { L1, L2, L5, L7 };
    state.alerts = state.alerts || [];
    for (const la of lebAlerts) {
      if (!state.alerts.some(a => a.text === la.alert.text)) state.alerts.unshift(la.alert);
      try {
        await fetch(`https://ntfy.sh/${process.env.NTFY_TOPIC || "godseye-alerts"}`, {
          method: "POST",
          headers: { "Title": la.title, "Priority": la.priority, "Tags": "rotating_light,lebanon" },
          body: la.body,
        });
      } catch (e) { console.warn("[WATCHDOG LB] ntfy failed:", e.message); }
    }
    if (lebAlerts.length) console.log("[WATCHDOG LB]", lebAlerts.length, "Lebanon alerts fired");
...
// === LEBANON ALERTS — see /PATCHES.md for full checkLebanonAlerts() function ===
